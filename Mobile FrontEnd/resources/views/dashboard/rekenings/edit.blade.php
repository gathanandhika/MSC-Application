...
<h1 class="h2">Edit Data Rekening</h1>
</div>

<div class="col-lg-8">
<form method="post" action="/dashboard/rekenings/{{ $rekening->id }}">
  @method('put')
  @csrf

  <div class="form-group mb-3">
      <label for="nama_bank" class="form-label">Rekening</label>
  ...
  <div class="mb-3">
    ...
    <input type="text" class="form-control @error('no_rekening') is-invalid @enderror" id="no_rekening" name="no_rekening" required  value="{{ old('no_rekening', $rekening->no_rekening) }}">
    @error('no_rekening')
      <div class="invalid-feedback"> {{ $message }} </div>
    @enderror
  </div>
