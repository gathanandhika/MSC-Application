...
import kotlinx.android.synthetic.main.msc2.*
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class Msc2Activity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.msc2)
    ...
    private fun datalapangan(){
        ...
        SetAPI.instance.msc2(token = "Bearer ${sessionManager.fetchAuthToken()}"
        ).enqueue(object : Callback<LapanganData> {
            override fun onResponse(call: Call<LapanganData>, response: Response<LapanganData>) {
                val response = response.body()!!
                tvlap2nama.text = response.nama_lapangan
                tvlap2harga.text = "Harga   : " +response.harga.toString()
                tvlap2detail.text = "Alamat : " +response.detail
            }
